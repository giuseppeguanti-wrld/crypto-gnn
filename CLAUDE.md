...
- **`project-thesis/`** — (in costruzione) l'eventuale implementazione sperimentale/codice del progetto (dati, grafo dinamico, modello GNN, baseline VAR, ecc.), corrispondente a quanto descritto come "Capitolo 6 — studio di caso" in `latex-thesis/chapters/06-case-study.tex`. Avrà un proprio `CLAUDE.md` quando la struttura sarà definita.
...
- `project-thesis/CLAUDE.md` (quando esisterà) verrà caricato allo stesso modo per i file dentro `project-thesis/`.
...
- `.gitignore` in root copre già entrambe le sottocartelle (pattern Python per `project-thesis/`, pattern LaTeX per `latex-thesis/`): quando si aggiungono nuovi strumenti/librerie, aggiornare `.gitignore` in root, non crearne uno per sottocartella.
